...
public class Map {
		...
		for (int y = 0; y < tiles.length; y++) {
			for (int x = 0; x < tiles[y].length; x++) {
				...
				if (setUnCol == 0) {
					...
					tiles[y][x] = new Tile(r.nextInt(3) + 1, x * tileSize, y
							* tileSize);
				...
				// tiles[x][y] = new Tile(r.nextInt(5), x * tileSize, y *
				// tileSize);
			}
		}
	...
	public void render(Graphics g) {
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				if (Math.round(tiles[x][y].getY()
						+ Camera.getCamera().getYOff()
						+ Art.getTileSheet().getCellSize()) < 0
						|| tiles[x][y].getY() + Camera.getCamera().getYOff() > BasicGame.size.height)
					continue;
				else if (tiles[x][y].getX() + Camera.getCamera().getXOff()
						+ Art.getTileSheet().getCellSize() < 0
						|| tiles[x][y].getX() + Camera.getCamera().getXOff() > BasicGame.size.width)
					continue;
				g.drawImage(tiles[x][y].getTexture().getImage(),
						Math.round(Camera.getCamera().getXOff())
								+ (int) tiles[x][y].getX(),
						Math.round(Camera.getCamera().getYOff())
								+ (int) tiles[x][y].getY(), (int) tileSize,
						(int) tileSize, null);
			}
		}
